...
async function createMigrationTable(){
  await client.queryArray`CREATE TABLE IF NOT EXISTS migrations (name varchar(255) UNIQUE, migrated_at timestamp default now());`
}
...
for await (const entry of Deno.readDir(migrationsDir)){
  ...
  const content = await Deno.readTextFile(posix.join(migrationsDir, entry.name))

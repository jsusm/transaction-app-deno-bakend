...
import { createTransactionSchema, updateTransactionSchema } from "./transaction.model.ts";
...
router.patch("/:id", isAuthenticated, async (ctx) => {
  ...
  const transaction = await transactionRepository.update(body, parseInt(ctx.params.id))
